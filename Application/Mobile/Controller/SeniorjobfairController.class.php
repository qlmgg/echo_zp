...
class SeniorjobfairController extends MobileController{
    // 初始化函数
    ...
    public function reserve(){
        ...
        $booth_status = 0;
        if(C('visitor.utype')==1){
            $booth_info = D('Seniorjobfair/JobfairExhibitors')->where(array('jobfair_id'=>$id,'uid'=>C('visitor.uid')))->find();
            if($booth_info){
        ...
        $img = D('Seniorjobfair/JobfairPositionImg')->where(array('jobfair_id'=>$id))->select();
        $position_img = array();
        foreach ($img as $key => $value) {
            $arr['src'] = C('qscms_senior_jobfair_site_domain').att($value['img'],'jobfair');
            $arr['w'] = 750;
            $arr['h'] = 400;
            $position_img[] = $arr;
        }
    ...
    public function reserve_save(){
        ...
        $r = D('Seniorjobfair/Jobfair')->jobfair_booth(C('visitor'),$jobfair_id,$position_id);
